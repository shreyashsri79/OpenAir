...
| D-62 | — | What carries rendezvous and relay control messages? | **accepted** — length-prefixed protobuf over mutual TLS, server pinned by DeviceID |
...
| D-71 | — | How does an Owned proof reach a capability that opens a stream? | **accepted** — on that stream, as its first frame; answers D-57's gap |
| D-72 | — | What does a remotefs path look like? | **accepted** — `root/sub/path`; the empty path lists the roots |
| D-73 | — | What does a source say about a path that is not there? | **accepted** — `NOT_FOUND` (0x0c), new in §10 |
| D-74 | — | Is a peer's trust level fixed for the life of a session? | **accepted** — no; re-read per message, so a grant and a revocation both land at once |
| D-75 | — | What level do notifications run at? | **accepted** — Trusted, on the identity key; the filter is what protects content |
| D-76 | — | Where does notification filtering live? | **accepted** — on the source, before anything is marshalled; never on the wire |
| D-77 | — | How does clipboard sync avoid a loop? | **accepted** — applied-digest suppression plus `origin_ts` last-writer-wins |
| D-78 | — | What does a streaming client read ahead, and when does it stop? | **accepted** — a window of prefetchFactor × BDP, abandoned on every seek |
| D-79 | — | How is the streaming block cache made safe to keep? | **accepted** — XChaCha20-Poly1305 under a per-process key, size-capped, wiped at start |
| D-80 | — | How does a media player open a remote file? | **accepted** — a loopback HTTP URL with a random token; Range becomes §11.2's read |
| D-81 | — | Is a read deadline an edge or a state? | **accepted** — a state, checked before every wait; corrects M9's `path.Conn` |
| D-82 | — | How is an input datagram authorised, with no room for a proof? | **accepted** — by the §6.3 announce that precedes it, plus the level re-read per event |
| D-83 | — | May a datagram capability start as soon as it has announced? | **accepted** — no; §6.3 gains an acknowledgement, because streams and datagrams are unordered |
| **D-84** | **ADR-4** | **What carries the media plane?** | **accepted** — stream-per-frame with `RESET_STREAM`; measured, and it closes D-9 |
| D-85 | — | Where do capture and encoding happen? | **accepted** — an external encoder process, not a cgo binding |
| D-86 | — | Does a *successful* pairing need the linger too? | **accepted** — yes; extends D-46, which only covered the exchange that fails |
| D-87 | — | What is the desktop GUI built on? | **accepted** — a second front end onto the daemon IPC, in the main module; it implements no protocol of its own |
| D-88 | — | What happens to the Android v1 stack? | **accepted** — deleted; the v2 Compose shell over the gomobile core becomes the only launcher entry |

**Open right now:** nothing that gates a milestone — D-9 is closed by D-84. Linux tier 1 — TPM-sealed privilege keys — is deferred by D-61, which leaves the maintainer's own machine on tier 2 despite having a TPM. D-10 is settled by D-31. Awaiting hardware: on-device Android throughput and battery (runbook in `oabench/androidkit/`). The Windows baseline is deferred to Phase 2 by D-32, though it remains a hard Phase 1 *exit* blocker. One optional refinement is flagged for the maintainer in D-30 — ephemeral per-peer delegation keys, which would make per-peer scope cryptographic rather than policy-enforced. Every decision gating Phase 1 is made and the trust store schema is fully determined. ADR-3 is fully resolved across D-18, D-19, D-20 and D-21, so the trust store schema is unblocked. D-16's queueing worry is answered by D-17; the port has now landed (D-35, D-36), so what remains is comparing BBRv1 against D-17's Cubic baseline.

**Raised by wave 1, needing the maintainer rather than an agent.** Three security questions from the threat model (D-43) remain: the D-20/D-21 conflict over whether a TPM-sealed key survives cold theft, the trust store's unspecified at-rest integrity, and DeviceID as a permanent tracking identifier. The fourth, `RelayAuth`'s missing domain separation, is answered by D-65. Separately, `PROTOCOL.md` needs four edits that wave 1 found and worked around rather than fixed: §8.3's chunk-offset semantics (D-40, the urgent one — it is wire-visible and two implementations choosing differently will corrupt files), §3 versus §10 on unknown versions (D-38), §3's claim that every enum is offset (D-39), and Appendix A gaining a public-key field (D-42). Two interface gaps are also outstanding: `identity.Identity` does not declare `ProtectionTier()` though Hello carries one, and `session.Handler` has no version accessor, which leaves §4's per-capability version negotiation degenerate.
...
flowchart TD
    ...
    Q5 ==chosen==> D9["D-84 · stream-per-frame with RESET_STREAM, measured.<br/>Level on a LAN; on the relayed profile datagrams<br/>are 3x the latency and lose frames outright,<br/>because one lost fragment loses a whole frame"]:::accepted
    ...
    Q13{"§11.4 / M11<br/>How does a remote file play like a local one?"}:::question
    Q13 ==chosen==> D80["D-80 · the daemon serves it on a loopback HTTP URL.<br/>Every player already speaks Range, and a Range<br/>request is §11.2's read with another header on it —<br/>so no plugin, nothing mounted"]:::accepted
    D80 --> D78["D-78 · read ahead 4 × the bandwidth-delay product,<br/>capped at 16 MiB, and abandon it on a seek.<br/>Cancelling is what makes the first byte after a<br/>seek one round trip instead of a window"]:::accepted
    D78 --> D79["D-79 · the block cache is XChaCha20-Poly1305<br/>under a key that exists only in memory,<br/>capped and wiped at start: it holds<br/>somebody else's files"]:::accepted
    D80 -.rejected.-> R13A["a FUSE mount<br/>a kernel module per platform, and Windows<br/>has no answer at all; players want a URL<br/>more than they want a path"]:::rejected
    D80 -.constrained by.-> D80N["loopback is not a trust boundary.<br/>Any local process can connect, so the<br/>160-bit token in the URL is the access control"]:::evidence
    D80 --> D85["D-85 · the same URL trick carries M15's screen:<br/>capture and encode are an ffmpeg subprocess,<br/>and the viewer is whatever player the user<br/>already has. No libavcodec in this process"]:::accepted

    Q14{"§13 / M14<br/>What authorises a keystroke<br/>sent as a datagram?"}:::question
    Q14 ==chosen==> D82["D-82 · §6.3's SessionAnnounce carries the Owned proof,<br/>and events are accepted only while it stands.<br/>A datagram has no room for an AuthProof and<br/>no msgType to bind one to"]:::accepted
    D82 --> D83["D-83 · the announce is acknowledged before the first<br/>event. A datagram is not ordered against the<br/>control stream, so events sent immediately after<br/>an announce arrive before it and are discarded"]:::accepted
    D82 -.rejected.-> R14A["a proof per event<br/>an Ed25519 signature per pointer move,<br/>on both sides, for a 15-byte datagram"]:::rejected
    D82 -.constrained by.-> D82N["the level is re-read per datagram (D-74),<br/>so revoking Owned stops input at the next<br/>event rather than the next announcement"]:::evidence

    Q15{"D-29 / D-51<br/>What does a graphical shell talk to?"}:::question
    Q15 ==chosen==> D87["D-87 · the daemon IPC, the same control plane<br/>the CLI drives. A window that is wrong about<br/>the protocol becomes impossible; the worst<br/>it can be is wrong about a label"]:::accepted
    D87 --> D88["D-88 · the phone answers the same way through<br/>the gomobile binding (D-31). One core, three<br/>shells, and the v1 TCP stack is gone rather<br/>than kept beside it"]:::accepted
    Q15 -.rejected.-> R15A["a GUI with its own engine, as v1.0 had<br/>two implementations of one protocol,<br/>and the one nobody tests is the one<br/>a user reaches first"]:::rejected
    D87 -.constrained by.-> D87N["subscribing with prompts is not optional:<br/>without a subscriber the daemon refuses<br/>inbound files, so the window is also<br/>what `openair watch` was for a terminal"]:::evidence
...
## D-62: Rendezvous and relay control frames are length-prefixed protobuf over mutual TLS, and the server is pinned by DeviceID
Date: 2026-07-31
Status: accepted (implements PROTOCOL.md §16)
Context: §16 defines `Registration`, `LookupRequest` and `LookupResponse`, and says the server "MUST verify the signature against the registering key before storing". It specifies no transport, no framing, and — the load-bearing omission — no way for the server to obtain that key. A DeviceID is `base32(SHA-256(pubkey)[0:10])` (§2), which is a hash: a server holding a DeviceID and a signature cannot recover the key to check the signature with.
Decision: Three things, all of which §16 leaves open.
- **Framing.** `msgType u16 || length u32 || payload`, little-endian per §0, capped at 64 KiB. Not the §3 envelope: that frame carries a capID and a negotiated protocol version, and neither exists on a connection spoken to a server rather than to a peer.
- **Transport.** TLS 1.3 with *both* ends presenting the certificate keyed by their identity key — the same construction peers use (D-7), reusing `identity.TLSConfigPairing` rather than a second TLS story.
- **How the server gets the key.** From the client certificate. The server derives the DeviceID from it and requires the registration to be for that device, so a registration is bound to its author by the connection as well as by its signature. No key field is added to `Registration`, and §16 needs no wire change.
Rationale: the client certificate answers the unanswerable question without touching the schema, and it is strictly stronger than a key field would be: a key in the message would prove only that *somebody* holds it, where the certificate proves the party on this connection does. Clients pin the server by DeviceID for the same reason peers pin each other — an address is not an identity, and a lookup answered by whoever won a race for the name is a device that cannot be reached. Endpoints are not secret, so this is an availability property rather than a confidentiality one, and it is worth stating that way.
Alternatives considered: adding `identity_public_key` to `Registration` (rejected — a wire change to solve what the handshake already solves, and it would let an unauthenticated party submit registrations on behalf of a key it merely knows). Reusing the §3 envelope (rejected — three fields with no meaning here, and a capID that Appendix B would then have to reserve). HTTPS with a JSON or protobuf body (rejected — a third serialisation and an HTTP server for two message types). WebPKI certificates for the server (rejected — it would put a certificate authority in a design that has none, and self-hosting is meant to be trivial).
Consequences: `openair-rendezvous` prints the DeviceID to pin when it starts, and the daemon flag is `--rendezvous host:port@deviceid` because the two halves are useless apart. A client that loses the pinned ID cannot safely use the server, which is the intended failure. PROTOCOL.md §16 gains the framing and the client-certificate requirement.

## D-63: The registration signing input length-prefixes its variable-length fields
Date: 2026-07-31
Status: accepted (corrects an ambiguity in PROTOCOL.md §16)
Context: §16 gives the signed input as `"openair-rendezvous-v1" || device_id || endpoints (in order) || relay_home || issued_at || expires_at`. Concatenated raw, that is ambiguous: endpoints `["a", "bc"]` and `["ab", "c"]` produce identical bytes, and so do `endpoints=["a"], relay_home="b"` and `endpoints=["ab"], relay_home=""`. One signature would then cover several different registrations.
Decision: each variable-length part is preceded by a u32 little-endian length — the endpoint count, then each endpoint, then the relay home. `identity.RendezvousSigningInput` is the only place the layout is written, and both the signer and every verifier call it.
Rationale: the attack is small but real. An endpoint list that can be re-split under one signature means a party that can modify the message — the server, or anything between it and a client — can move a device's advertised port between fields without invalidating the signature. Length prefixes cost eight bytes on a message sent every few minutes.
Alternatives considered: a delimiter such as NUL between fields (rejected — it moves the problem to escaping, since an endpoint containing the delimiter would still be ambiguous). Signing the serialised protobuf instead (rejected — protobuf serialisation is not canonical, so two encoders can produce different bytes for the same message and the signature would depend on which library wrote it).
Consequences: §16's pseudocode needs the same amendment §6's did not, and until it is made this implementation's signatures will not verify against one written from the document alone. `TestSigningInputIsUnambiguous` fails if the prefixes are removed, which is what keeps the fix from being optimised away later.

## D-64: A relay data frame names the destination inbound and the source outbound
Date: 2026-07-31
Status: accepted (corrects PROTOCOL.md §17)
Context: §17's data frame is `dst_device_id (16) || length (u32) || payload`, in both directions. On the client-to-relay direction that is right. On the relay-to-client direction it cannot be: a client only ever receives frames addressed to itself, so a destination field there carries no information, and the receiver is left unable to tell which peer a packet came from.
Decision: the 16-byte field is the **destination** on the way in and the **source** on the way out. The frame layout, size and parsing are unchanged; only the meaning of the field differs by direction.
Rationale: attribution is not optional once the payload is a QUIC packet. D-6 puts one QUIC connection per peer, and a QUIC stack demultiplexes by the address its packet conn reports — `relay.PacketConn.ReadFrom` has to return *somebody*, and returning our own DeviceID would collapse every peer onto one apparent address. The alternative designs all cost more: a second field widens the frame for every packet, and a per-peer connection to the relay multiplies sockets by the number of peers.
Alternatives considered: adding a `src_device_id` field (rejected — 16 bytes on every packet to carry what the direction already determines). One relay connection per peer (rejected — a device with eight paired peers would hold eight TLS connections to the same relay, and the relay would hold them too). Letting the QUIC connection ID disambiguate (rejected — quic-go routes by remote address before it looks at connection IDs, so the packet conn has to answer first).
Consequences: §17 needs the sentence. An implementation written from the document alone would send correct frames and misattribute every received one, which is the kind of defect that shows up as "it works with one peer" — worth fixing in the spec rather than only here.

## D-65: Relay authentication is domain-separated and length-framed — closes one of D-43's open questions
Date: 2026-07-31
Status: accepted (answers D-43, corrects PROTOCOL.md §17)
Context: §17 says a client authenticates by signing "both nonces" and says no more. D-43 raised the missing domain separation as one of four security questions for the maintainer; this implements the answer rather than leaving it open. There are now three things an identity key signs — an Owned `AuthProof` (§6), a rendezvous `Registration` (§16) and a relay challenge (§17) — and without separation they are three bare byte strings signed by one key.
Decision: the signed input is `"openair-relay-v1" || device_id || len(client_nonce) || client_nonce || len(server_nonce) || server_nonce`. Both nonces are 32 bytes, matching §6's, and each is length-prefixed for the same reason §16's endpoint list is (D-63).
...
## D-71: An Owned capability sends its AuthProof on the stream it authorises
Date: 2026-08-01
Status: accepted (answers the gap D-57 recorded, corrects PROTOCOL.md §11)
Context: D-57 settled how an `AuthProof` reaches its operation on the control stream — it precedes the message and is spent by it — and named the case it does not cover: a capability that opens its own stream. QUIC does not order a new stream against the control stream, so a proof sent on the control stream and a stream opened immediately after can arrive in either order. D-57 could leave that open because no Phase 1 capability declared Owned. remotefs (§11) declares Owned and is one stream per request, so it is exactly the case.
Decision: the proof is the first envelope on the capability stream — capID 0, `AUTH_PROOF` — followed by the request envelope for the operation it authorises. `session.OpenOwnedStream` writes the pair; `serveStream` reads a leading proof into the same pending set the control path uses, and the operation that follows spends it. A second control message where the request should be is a protocol violation, so a proof cannot chain to another proof.
Rationale: QUIC orders bytes within a stream, so a proof written first on the stream it authorises is *always* seen first. Nothing else needs to change: the verification, the ±60 s window, the nonce cache and the once-only spend are the same code as the control path, so there is one implementation of §6 rather than two.
Alternatives considered: sending the proof on the control stream and having the receiver wait briefly for it (rejected — a timing rule pretending to be an ordering rule, and the wait would be either flaky or slow). Giving each capability stream a request ID and matching proofs to it (rejected — the envelope has no such field, and adding one to fix an ordering problem that stream ordering already solves is the wrong trade). Making remotefs Trusted (rejected — reading someone's filesystem unattended is precisely what §6 gates).
Consequences: §11 gains the paragraph. An Owned capability that opens a stream and forgets the proof is refused with UNAUTHORISED rather than working intermittently, which is the failure mode worth having. The `ProtocolError`-carrying variant of `StreamErrorCode` also means a capability can read the refusal over a non-QUIC transport, which is what its own tests run on.

## D-72: A remotefs path is `root/sub/path`, and the empty path lists the roots
Date: 2026-08-01
Status: accepted (fills a gap in PROTOCOL.md §11.1)
Context: §11.1 says roots are configured on the source and that escaping them is UNAUTHORISED, but never says what a path in a request looks like. It cannot be a local filesystem path — the source exposes roots, not a filesystem, so `/etc/passwd` is not a thing a client can ask for even in principle — and without a defined form every client would invent one.
Decision: the first component names a configured root, the rest is relative to it, separators are forward slashes only. The empty path (also `/` and `.`) is the list of roots. A root is named by the base name of its directory, or explicitly with `--share name=/path` when two would collide. Traversal is checked syntactically before any filesystem call and again after resolution, because a symlink inside a root can point out of it and syntax cannot see that.
Rationale: the empty path is the part worth arguing for. Without it a client has no way to find out what it may browse, and every UI would have to be handed the share names by some other channel — for a capability whose entire purpose is browsing. With it, `openair ls DEVICE` is a complete first command.
Alternatives considered: absolute local paths with a prefix check (rejected — leaks the source's directory layout, and makes every client construct paths it cannot validate). A separate `ListRoots` message (rejected — one more message type for something an empty path already expresses). Opaque root IDs (rejected — unreadable in a CLI, and the names are not secret from a peer already allowed to read the files).
Consequences: §11.1 needs the paragraph. A source that renames a share breaks a client's stored paths, which is the same behaviour as renaming a directory and is understood.

## D-73: §10 gains NOT_FOUND
Date: 2026-08-01
Status: accepted (extends PROTOCOL.md §10)
Context: §11 gives remotefs no in-band error message, so the only channel a source has for a failure is the stream reset code. §10's list has nothing for "that path does not exist" — the nearest are `REJECTED`, which it defines as "user declined", and `RESOURCE_EXHAUSTED`. For a file browser, a missing path is the single commonest failure there is.
Decision: add `0x0c NOT_FOUND`, operation-fatal.
Rationale: the two codes it would otherwise collapse into send a user to different places. `REJECTED` says somebody saw a prompt and said no; `RESOURCE_EXHAUSTED` says try later. Neither is "you typed a name that is not there", and a client cannot distinguish them from prose it does not have.
Alternatives considered: reusing `REJECTED` (rejected — it means a human declined, and a client would tell the user to go and look at a device where nothing was asked). Adding an error message to every remotefs response (rejected — a response shape per operation carrying a status is a larger change to §11 than one code is to §10). Returning an empty result for a missing path (rejected — indistinguishable from an empty directory).
Consequences: 0x0c is now taken. Implementations written against the older table see an unknown operation-fatal code, which §10's own model already accommodates — the operation fails either way, and only the explanation is lost.

## D-74: A peer's trust level is re-read per message, not cached for the session
Date: 2026-08-01
Status: accepted (corrects an M6 implementation choice)
Context: Found by hand, running M10 on two real daemons: pairing, then `openair trust --owned` on the source, then browsing — refused. The level had been read into the session record during Hello (M6), and the promotion made afterwards did not reach a session already open. The mirror image is the one that matters: a *demotion* was equally invisible, so a peer that had been dropped from Owned kept acting Owned until something reconnected. §6.1 requires revocation to land on a session already running, and it did for the explicit `Revoke` message and the unlock, but not for a local change of level.
Decision: `authorizeInbound` consults `Config.PeerLookup` for the current level, pinned privilege key and granted capabilities on every message that needs them, falling back to the record from Hello when no lookup is supplied.
Rationale: a trust level is state on this device, changed by its user while sessions are open, and the file it lives in is the authority. Caching it turns a security decision into a snapshot taken at an arbitrary earlier moment. The cost is one map lookup per authorised message, against a trust store already held in memory and already taken under a lock.
Alternatives considered: closing every session with a peer whose level changed (rejected — a revocation should not need to break a running transfer to take effect, and a promotion certainly should not). An invalidation callback from the trust store into each session (rejected — more machinery, more state, same answer). Leaving it, and documenting that a promotion needs a reconnect (rejected — that is the harmless half; the harmful half is silent).
Consequences: `openair trust` now takes effect immediately in both directions, which is what a user reasonably expects from a command that prints a sentence in the present tense. Verified by hand: demoting a peer mid-session makes its next browse fail, with no reconnection anywhere.

## D-75: Notifications run on the identity key, at Trusted
Date: 2026-08-01
Status: accepted
Context: §12 does not say what trust level notification mirroring needs. The two candidates behave very differently: at Owned every message needs an AuthProof, so mirroring stops the moment an unlock lapses; at Trusted it works for any paired device, continuously.
Decision: Trusted, on the identity key, exactly like the clipboard (D-20).
Rationale: this is the same argument D-20 made and it applies more strongly here. Notification mirroring is a background flow a person stops noticing until it breaks, and a version that goes silent every six hours until someone re-authenticates would be experienced as unreliable rather than as secure — and the reaction to that is to turn it off, which protects nothing. What actually protects the content is the source-side filter (D-76), which withholds by app before anything is sent, and the fact that a peer had to be paired at all.
Alternatives considered: Owned (rejected — see above; it also makes desktop-to-desktop forwarding, PRD R23, need an unlock on a machine nobody is sitting at). A per-category level, Owned for "msg" and Trusted for the rest (rejected — the category is the source's claim, so it would be an access-control decision made by the party being controlled).
Consequences: a paired device sees whatever the filter allows, with no further ceremony. A user who wants a narrower rule expresses it as an allowlist rather than as a trust level, which is the control that actually names what they care about.

## D-76: Notification filtering is source-side and never appears on the wire
Date: 2026-08-01
Status: accepted (implements PRD R22 as §12 requires)
Context: §12 and R22 both say filtering happens on the source, and §12 explicitly declines to define a filter-configuration message. It is worth recording why, because "let the sink say what it wants" is the obvious design and it is wrong.
Decision: `Config.Allow` is consulted before a `Posted` is constructed. `AllowList` is the shape a phone should use — everything private until named — and `BlockList` exists for a desktop forwarding its own notifications, where being made to enumerate every application first means nobody turns it on. The daemon exposes both as `--notify-allow` and `--notify-block`, with allow winning if both are given.
Rationale: a filter negotiated over the wire is a filter applied after the content has left the device that was supposed to withhold it. The test that matters is not "was it delivered" but "did the bytes exist at all", and `TestFilteredContentNeverReachesTheWire` asserts exactly that, against the bytes a spy session recorded.
Alternatives considered: a sink-declared subscription (rejected — the content has already crossed by the time the sink's preference is honoured, so a relay operator and any future compromise of the sink both see what was meant to be withheld). Filtering in the daemon's fan-out rather than in the capability (rejected — then a caller using the capability directly, which the mobile binding does, has no filter at all).
Consequences: changing what is mirrored is a local configuration change on the device that holds the notifications, and takes effect at the next post. There is deliberately no way for one device to ask another to widen its filter.

## D-77: Clipboard loop suppression is an applied-digest check plus origin_ts
Date: 2026-08-01
Status: accepted
Context: M13 turns M5's push into something a watcher sends, and two devices watching each other is a loop: A applies what B sent, A's watcher sees a change, A sends it back. §9 provides `origin_tag` and `origin_ts` and does not say how to use them. PRD K3 flags the harder half — simultaneous edits.
Decision: three rules. (1) Content received from a peer is recorded as "applied" *before* it is written to the OS clipboard, and the watcher never sends content whose digest matches; recording before the write matters because the write is a subprocess and a poll landing in between would see unexplained peer content. (2) `origin_tag` drops a device's own push echoed back, which M5 already implemented. (3) A simultaneous edit is settled by `origin_ts`: content older than this device's own last local copy is not applied, so two devices that copy at the same instant converge on the later one instead of swapping forever.
Rationale: the digest check is what stops the ordinary loop and costs nothing. The timestamp rule is the only one that needs a clock, and it needs it only to order two events seconds apart — the failure mode under skew is one copy being lost, not a loop, which is the right way round.
Alternatives considered: a sequence number per device with vector-clock merging (rejected — a clipboard is a single last-writer-wins cell, and this is a lot of machinery for one cell). Suppressing by content alone with no timestamp (rejected — two simultaneous edits then swap clipboards and each keeps sending, which is the loop with extra steps). Not syncing automatically at all (rejected — R19 asks for it, opt-in, which is what it is).
Consequences: auto-sync is opt-in behind `--auto-clipboard`, because a device that mirrors every copy also mirrors what a password manager put on the clipboard. Polling is once a second — one short-lived subprocess, since D-54 puts the clipboard behind helpers — and backs off to fifteen seconds when the clipboard cannot be read at all, which is what a display that went away looks like.

## D-78: A streaming client reads ahead by the bandwidth-delay product, and abandons it on a seek
Date: 2026-08-01
Status: accepted (implements §11.4)
Context: §11's bargain is a dumb server and a smart client: the source answers one range per stream and knows nothing about what it is serving, so every property M11 promises has to be produced on the client. §11.4 asks for a read-ahead window of "roughly RTT × observed bitrate" and says nothing about what happens when the position moves, which is the case that decides whether seeking feels instant.
Decision: `remotefs.Reader` fetches whole 1 MiB blocks, up to four range reads in flight, with a window of `prefetchFactor` (4) times the bandwidth-delay product, floored at 4 blocks and capped at 16. The bitrate in that product is measured from completed reads, falling back to `PathInfo.BandwidthBytes` until one has completed. Every read recomputes the window and **cancels every in-flight fetch outside it**, which is what a seek does.
Rationale: the BDP is the amount that has to be in flight to keep the path busy at all; read-ahead needs a multiple of it because a player reads in bursts. The floor is for a LAN, where the BDP rounds to nothing and a player still wants a few megabytes of slack; the cap is for a relayed path, where the BDP is large and prefetching 200 MB of a film someone is about to seek away from is waste. The cancellation is the part that matters and it is cheap only because of §11.2's shape: one request per stream means abandoning a read is a stream reset that drops exactly one range. Without it, the first byte after a seek costs however much was already queued for the old position — measured at five round trips in `TestASeekAbandonsTheReadAheadForTheOldPosition`, against one with it.
Alternatives considered: a fixed window (rejected — the same number is either useless on a LAN or ruinous on a relayed path, and §11.4 asks for adaptation by name). Letting the old read-ahead drain rather than cancelling it (rejected — it makes seek latency a function of window size, which is exactly backwards). Read-ahead in the source (rejected — it is the thing §11 exists to refuse; a source that guessed would be wrong for every client that was not a player).
Consequences: a seek costs one round trip plus whatever the source takes, and the cancelled work is wasted bandwidth — bounded by the window, which is why the cap exists. The reader is `io.ReadSeeker` and `io.ReaderAt`, so `http.ServeContent` drives it directly (D-80).

## D-79: The streaming block cache is encrypted under a per-process key and wiped on start
Date: 2026-08-01
Status: accepted (implements §11.4, PRD K8)
Context: §11.4 makes a client-side cache optional and then attaches two conditions that are not: encrypted at rest, and size-capped. Both follow from what it holds — blocks of another device's files, written to this device's disk because someone browsed, without anyone deciding to save anything.
Decision: `remotefs.Cache` is an LRU of blocks on disk, each sealed with XChaCha20-Poly1305 under a 32-byte key generated at `NewCache` and never written down, with the file name as associated data. It is capped (512 MiB for the daemon's streaming cache), `NewCache` removes anything already in the directory, and `Close` removes the directory.
Rationale: the alternative — deriving the key from the device key so a cache survives a restart — buys a warm cache after a reboot and costs a durable, decryptable copy of someone else's files sitting in a directory indefinitely. A streaming cache is worth very little cold, so the trade is easy. The file name as associated data means a block cannot be moved to another path's key and still open, which is the failure a rename would otherwise cause silently. The cap is enforced on the sealed size, because that is what is actually on the disk.
Alternatives considered: an in-memory cache only (rejected — the point is files larger than memory; a 40 GB film seeked around in for an hour is what this is for). A key derived from the identity key (rejected — see above). No cache at all (rejected — a player that re-reads the same 1 MiB while a user scrubs back and forth would re-fetch it every time).
Consequences: a restarted daemon streams cold, which costs one window of read-ahead. Nothing readable is left on disk when it stops, and nothing readable is left there if it is killed instead.

## D-80: A remote file reaches a media player as a loopback HTTP URL
Date: 2026-08-01
Status: accepted
Context: M11's stated deliverable is playing a remote video and seeking instantly. `remotefs.Reader` makes that possible; something still has to connect it to a program that plays video, and the protocol has nothing to say about the local half.
Decision: `openair stream DEVICE PATH` asks the daemon to publish the file at `http://127.0.0.1:PORT/s/TOKEN/NAME` and prints the URL. `http.ServeContent` serves it from a fresh `remotefs.Reader` per request, so a `Range` header becomes a seek and the seek becomes §11.2 range reads. The server is created on first use, listens on loopback only, drops a URL after fifteen idle minutes, and dies with the daemon. `--open` and `--with` hand the URL to a player.
Rationale: every media player already opens URLs and already issues Range requests, and a Range request is §11.2's read with a different header on it. Nothing has to be mounted, no plugin exists, and the seek path is the player's own. The alternative that looks tidier — a FUSE mount — is a kernel-level dependency per platform with no answer on Windows, for a worse result.
Alternatives considered: FUSE or WinFsp (rejected — see above; also makes every `stat` in a directory listing a network round trip inside a syscall a program cannot cancel). Downloading to a temporary file and playing that (rejected — it is `openair get`, and it means waiting for 40 GB to watch the last five minutes). A local socket protocol of our own (rejected — nothing would speak it).
Consequences: loopback is not a trust boundary — any local process can connect — so the 160-bit token from `crypto/rand` in the path is the access control, and the URL is the secret rather than the path. Nothing is published until someone asks, each URL names exactly one file, and the shell that asked is the only thing that is told. A stream held open across a network change re-resolves the session per request rather than capturing one, so it follows the device the way M9 intends.

## D-81: A read deadline is a state a reader checks, not an edge it must be present for — corrects M9
Date: 2026-08-01
Status: accepted (fixes a defect in D-69's packet conn)
Context: `path.Conn.SetReadDeadline` woke a blocked `ReadFrom` by closing a channel and installing a fresh one. That is edge-triggered: it only reaches a reader that is already waiting. A reader that arrives *after* the edge waits on the new channel, which nobody will ever close, with a deadline that has already passed. Found while running the daemon suite for M11 — it hung for fourteen minutes at nought CPU, and the stack said `Endpoint.Close` was inside `quic.Transport.Close`.
Decision: `ReadFrom` checks whether the deadline has passed *before* it waits, and again when it is woken. The channel remains as the wake-up mechanism; the deadline itself is now state rather than an event.
Rationale: quic-go's `Transport.Close` unblocks its read loop by setting a deadline in the past and then waiting for the loop to return. With an edge-triggered deadline the loop can miss that edge — it is in `handlePacket`, not in the select — and then blocks forever on a channel that will never close. `Transport.Close` waits for it, `Endpoint.Close` waits for `Transport.Close`, and the daemon never shuts down. That is a real shutdown hang and not merely a missed timeout, which is why it appeared as an unrelated test hanging rather than as a slow one: `t.Cleanup` was waiting on a daemon that could not stop. `net.Conn`'s contract also reads this way — a deadline is a time after which reads fail, not a notification — so this is what a caller substituting a real socket already expects.
Alternatives considered: closing the channel without replacing it and creating a new one only in `SetReadDeadline` (rejected — it narrows the window rather than removing it; a reader between the close and the next `SetReadDeadline` still misses). Waking readers with a `sync.Cond` (rejected — `ReadFrom` must also select on packets and on close, and a Cond does not compose with a select). A poll interval on the read (rejected — a wake-up per interval per conn, for a case that has an exact answer).
Consequences: `Transport.Close` returns promptly, and the daemon test package went from hanging indefinitely to 9 seconds. `TestAReadDeadlineIsNotJustAnEdge` sets a deadline while nothing is reading and asserts the next read still fails; `TestADeadlineWakesAReaderAlreadyWaiting` keeps the ordinary timeout honest.

## D-82: An input datagram is authorised by the session announcement that precedes it
Date: 2026-08-01
Status: accepted (implements §13 within §6.3)
Context: §13 puts input events on QUIC datagrams with a fixed 6-byte header and no room for anything else, and §6 requires Owned for anything unattended — which controlling somebody's keyboard certainly is. The two do not obviously meet: an `AuthProof` is a 32-byte nonce and a 64-byte signature bound to a capID and a msgType, and a datagram has neither a msgType nor the space.
Decision: the proof travels on the `SessionAnnounce` that §6.3 *already* requires before any use of `input`, verified by §6's existing gate because an announce is an ordinary control message. Events that follow are applied only while that announcement stands and the peer is still recorded as Owned, which is re-read per datagram. Ending the announcement — by the initiator, by a local user's kill, or by two minutes of silence — stops them and releases anything held.
Rationale: this is what §6.3 is for. It already says an initiator MUST announce before any use of input and that the accessed device MUST show an indicator for the lifetime of such a session, so the announcement is a session-shaped thing that already exists, is already ordered, and already has a lifetime. Attaching the proof to it costs one signature per session instead of one per pointer move. The per-datagram check that remains is the trust level, which is a map lookup and re-read anyway (D-74) — so revoking Owned stops input at the next event rather than at the next announcement, which for a capability that is *moving somebody's mouse* is the difference that matters.
Alternatives considered: a proof on every event (rejected — an Ed25519 signature per pointer move on both sides, and the proof would be four times the size of the event it authorises). A proof every N events or every N seconds (rejected — it is the same design with a window in which it is not true, and the window is the whole question). Carrying input on a stream instead so the existing per-stream proof (D-71) applies (rejected — §13 chose datagrams because a lost pointer sample must not be retransmitted late, and a stream would queue behind itself under loss, which is exactly the feel this milestone is trying to avoid).
Consequences: `--accept-input` is off by default, and a device that has not set it refuses the announcement rather than dropping datagrams silently (D-83's acknowledgement carries the reason). A local user's kill both ends the session and refuses another from that peer for five minutes — otherwise "kill" means the indicator flickers and comes back, which is not what PRD R4 asks for. The capability is still *registered* on every daemon, because §4's negotiation is symmetric and a controller that did not advertise capID 5 would find the device it is driving discarding its events.

## D-83: §6.3 gains an acknowledgement, because a datagram is not ordered against a stream
Date: 2026-08-01
Status: accepted (amends PROTOCOL.md §6.3; wire addition)
Context: with D-82 in place, M14's first attempt worked exactly once out of never. The announce goes out on the control stream and the events go out as datagrams a microsecond later; QUIC orders neither against the other, and the datagrams — smaller, unreliable, not waiting for anything — routinely arrive first. They were then discarded as unauthorised, correctly, and the person's keystrokes vanished.
Decision: add `SessionAnnounceAck` (control msgType 19) carrying `session_id`, `accepted` and `reason`. An initiator waits for it before its first datagram, for up to ten seconds. The accessed device sends it after recording the announcement, so an event arriving in the same instant already has something to be authorised against, and refuses in it — with a reason — when it will not honour what was announced.
Rationale: §6.3 never needed an acknowledgement because everything using it until now ran on streams, where the announce and the work are ordered by the stream itself. `input` is the first capability where that is not true, and the fix has to be a round trip: there is no way to order a datagram against a stream from inside QUIC. Making the refusal explicit is the second half of the value — a device with `--accept-input` off previously dropped every event in silence, which is indistinguishable from a broken network to the person driving.
Alternatives considered: buffering unauthorised events on the receiver for a grace period in case an announce turns up (rejected — it means holding keystrokes from an unauthorised peer, which is the wrong side of the fence to be storing them, and the grace is a guess). Delaying the first event by a fixed interval (rejected — a guess about RTT, wrong in both directions). Sending the first events on a stream and the rest as datagrams (rejected — two paths for one flow, and the ordering problem returns at the boundary).
Consequences: one extra round trip when a control session opens, and none per event; the initiator gets a legible refusal instead of silence. `SessionAnnounceAck` is new wire, so §6.3 and Appendix B's message list are amended rather than merely implemented, and an implementation that does not send one will find peers that use datagram capabilities refusing to start after ten seconds.

## D-84: ADR-4 resolved — the media plane is stream-per-frame with RESET_STREAM
Date: 2026-08-01
Status: accepted (closes D-9; makes PROTOCOL.md §14.2 normative)
Context: D-9 has been open since the first week. HLD ADR-4 leaned towards QUIC datagrams; D-24 moved the lean to streams by removing the scheduling argument (with bulk quiesced there is nothing for datagrams to be packed ahead of); §14.2 was written provisionally around streams and marked as requiring a spike before anything was built on it. The spike is `oabench mirror`, and it is what this entry records.
Decision: **stream-per-frame with `RESET_STREAM`**, as §14.2 describes. §14 stops being provisional.
Measurement: 60 fps, 8 Mb/s encoded, 15 s per run, keyframe every 60 frames, both framings, three shaped profiles, each alone and against a saturating bulk transfer on the same connection (`oabench/netem/mirror-matrix.sh`; raw rows in `oabench/netem/mirror-results-2026-08-01.jsonl`). Frame latency is capture instant to the moment the whole frame is readable at the sink, measured on one machine so both ends share a clock.

| profile | framing | p50 | p95 | p99 | frames lost |
|---|---|---|---|---|---|
| lan-1g | stream | 0.83 ms | 0.97 ms | 1.93 ms | 0 |
| lan-1g | datagram | 0.82 ms | 0.98 ms | 1.89 ms | 0 |
| wifi-5g | stream | 3.85 ms | 4.04 ms | 9.40 ms | 0 |
| wifi-5g | datagram | 3.78 ms | 4.57 ms | 17.93 ms | 9 |
| wan-relay | stream | 54.1 ms | 72.7 ms | 156.8 ms | 0 |
| wan-relay | datagram | 161.8 ms | 229.7 ms | 275.2 ms | 32 |

Rationale: on a LAN the two are indistinguishable, which is the case people quote when they say datagrams are the conventional choice for realtime video — and it is the case that does not decide anything. The decision is made on `wan-relay` (100 Mb/s, 80 ms RTT, 0.5% loss), where streams are three times faster at the median and lose nothing while datagrams lose 32 frames of about 900. The mechanism is not subtle: a keyframe at this bitrate is roughly 170 fragments, each of which is an independent chance to lose the whole frame, and the reassembler has no way to ask for the missing one. Streams get fragmentation, reassembly, retransmission of *the part that was lost*, and flow control from QUIC itself. The tail tells the same story from the other side: streams' p99 of 157 ms comes with 525 frames deliberately abandoned by the source, which is `RESET_STREAM` doing exactly what §14.2 designed it to do — dropping frames that could not have been shown in time instead of queueing them.
Alternatives considered: datagrams with application-level fragmentation (rejected — measured above; also the 32-entry send queue discards silently on overflow, which is a failure mode with no signal at all). A raw RTP/UDP sidecar, ADR-4's original fallback (rejected — it was always the escape hatch for "QUIC cannot hold latency", and QUIC held latency; taking it would add a second NAT-traversal surface and a second crypto surface to audit, which is precisely what one connection per peer exists to avoid). Stream-per-frame without resets (rejected — the p99 above is the reset working; without it a stalled frame delays every frame behind it).
Consequences: §14 is normative and M15 is unblocked. The `stale` counts on `wan-relay` — 525 of about 900 frames abandoned — are not a defect but they are a message: at 8 Mb/s on that path the source is encoding more than the path can carry, so `BitrateHint` (§14.1) is load-bearing rather than advisory, and a source that ignores it will show a viewer roughly half the frames. Two caveats on the measurement itself, in the spirit of D-33: it runs on upstream quic-go rather than the vendored fork, so BBR's behaviour under the same loss is not covered here, and the frames are synthetic (random bytes at a realistic size distribution) rather than an encoder's output, so this measures transport and not encode latency.

## D-85: Capture and encoding are an external process, and the viewer is a media player
Date: 2026-08-01
Status: accepted
Context: M15 needs H.264 out of a screen and a picture on the far end. Go has neither an encoder nor a decoder, and both ends of that are large C codebases.
Decision: the source runs an encoder as a subprocess and reads Annex-B H.264 from its standard output — ffmpeg by default, `--mirror-command` for anything else. The sink republishes the frames it reassembles at a loopback URL, and the viewer is whatever media player the user already has (`openair mirror laptop --with mpv`). Neither libavcodec nor a video window exists in this project.
Rationale: this is D-54's argument with more force behind it. A cgo binding to libavcodec would put a large codebase with a long CVE history inside the process that holds this device's keys, would have to be built per platform, and would be a build dependency for every user including those who never mirror anything. The subprocess is the same isolation the clipboard already uses, and the frame rate makes it *cheaper* than the clipboard's arrangement rather than dearer: one process for a whole session, not one per event. On the viewing side, mpv and ffplay already decode H.264 from a URL with hardware acceleration, sub-frame latency and years of work behind them; a window built here would be worse for as long as it existed. M11 established that a loopback URL is how this project hands media to a player (D-80), and this is the same idea with a live stream instead of a file.
Alternatives considered: cgo bindings to libavcodec (rejected — see above). A pure-Go encoder (rejected — none exists that is usable for realtime H.264, and writing one is a project rather than a milestone). Sending raw or JPEG frames to avoid an encoder entirely (rejected — 1080p30 raw is 1.5 Gb/s, and motion JPEG is roughly ten times H.264 for the same picture; both make the transport decision in D-84 irrelevant by drowning it). Building a viewer window (rejected — the same reasoning, in the other direction).
Consequences: a device with no encoder cannot share its screen, and says so at the moment a peer asks rather than failing halfway — `screen.Available` is checked before the session starts, and `openair mirror` turns the refusal into "install ffmpeg there". The frame boundary has to be recovered from a byte stream, which is why the encoder is asked to emit access unit delimiters and why `splitFrames` exists. And Wayland is a real gap: ffmpeg cannot open a desktop portal on its own, so a Wayland session needs `--mirror-command` naming a portal-capable producer (wf-recorder, or a PipeWire pipeline). X11, XWayland, Windows and macOS work with the built-in arguments.

## D-86: The linger applies to a pairing that succeeds, not only to one that fails — extends D-46
Date: 2026-08-01
Status: accepted (fixes a race D-46 half-fixed)
Context: D-46 established that QUIC's `CONNECTION_CLOSE` overtakes stream data that has not been flushed, and made this package wait 250 ms after writing a message that ends a conversation — a declined `PairConfirm`, a `Revoke`. The accepted `PairConfirm` was left out, because the exchange visibly worked. Found while stressing the suite for wave 8: `TestPairCommand_RoundTrip` failed twice in ten runs under load, and the mobile pairing test the same way. Each time one side reported "paired" and the other waited out its timeout.
Decision: linger after a successful `PairConfirm` as well, on the same 250 ms.
Rationale: §5.2's confirm exchange is symmetric and nothing acknowledges the last message of it. Both sides send `PairConfirm` and then wait for the peer's, so whichever side's confirm arrives second finishes first — and its caller closes the session immediately, which can discard its own confirm before the peer has read it. The peer is then one message short of pairing and waits two minutes for it. This is the same mechanism D-46 documented, and the reason it was missed is that the failing path was the one being debugged at the time: a decline is rare and was tested deliberately, while an accept works nearly always and fails only when the close beats the confirm.
Alternatives considered: an explicit acknowledgement for the final `PairConfirm` (rejected — it moves the problem rather than solving it, since the acknowledgement is then itself the unacknowledged last message). Waiting for the peer to close the session (rejected — both sides would wait for each other). Raising the test timeouts (rejected — the timeout was reporting a real defect; a user pairing over a busy WiFi network sees exactly this).
Consequences: pairing takes 250 ms longer on both sides, which is invisible next to reading six digits off a screen. Ten runs under load pass where two of ten failed before, and the mobile pairing path -- which is the same handler -- is covered by the same change.

## D-87: The desktop GUI is a second front end onto the daemon, not a second engine — retires D-26's split module
Date: 2026-08-02
Status: accepted
Context: v1.0's `openair-gui` was a Fyne app with its own `internal/sender` and `internal/receiver` — a complete second implementation of the transfer path, in its own Go module. It spoke the v1 TCP protocol and was deleted with the rest of the legacy code, which left the project with a CLI and a daemon and nothing a person could click. D-26 had kept `openair-gui` a separate module "until v1.0 retires"; v1.0 has now retired.
Decision: `cmd/openair-gui` in the main module, built on `internal/daemon.Client`. Every action in the window is one call on the same IPC control plane `openair` drives (D-29, D-51). It opens no QUIC session, holds no key, and knows nothing about the wire format.
Rationale: two implementations of one protocol is the defect v1.0 actually had — the GUI's sender and the CLI's sender drifted, and the one with fewer tests was the one most users reached first. A shell that can only make calls the CLI can make cannot disagree with the CLI about pairing, trust or framing; the worst it can be wrong about is a label. Keeping it in the main module follows from the same thing: a separate module needs a `replace` directive to see `internal/daemon` at all, and D-26's reason for the split (v1.0's independent engine) no longer exists.
Alternatives considered: a separate module again (rejected — nothing to isolate now that the engine is shared, and the `replace` directive is pure friction). Driving sessions directly, as the CLI's `--no-daemon` path does (rejected — that path exists so a transfer works with no daemon installed, which is not a state a GUI user is in; and browsing, streaming and mirroring are Owned-level and need the privilege key the daemon holds, so a direct-mode GUI would fail at exactly the features that motivate a GUI). A web UI served by the daemon (rejected — it makes the daemon serve HTTP to a browser on principle, and the loopback trust boundary in D-80 is a token on one URL, not an origin policy).
Consequences: Fyne enters the root `go.mod`, so `go build ./...` now needs cgo and system GL/X11 headers on Linux; the four network binaries still build without them, and CI does not gate the GUI (unchanged from v1.0, noted in functionality.md). On Linux the build wants `-tags x11` unless Wayland development headers are installed, because go-gl/glfw compiles both backends by default. The window subscribes with `prompts: true`, which makes it the graphical answer to `openair watch` — an inbound transfer can be approved by clicking rather than by leaving a terminal open, and a device with the GUI running is no longer a device that refuses files because nobody was watching.

## D-88: The Android app keeps only the v2 shell — the v1 TCP stack is deleted, not kept beside it
Date: 2026-08-02
Status: accepted (supersedes the two-launcher arrangement described under X5)
Context: `openair-android` shipped two stacks and two launcher entries: v1 over TCP on `_openair._tcp`, and the v2 Compose shell over the gomobile-bound Go core on `_openair._udp`. They could not talk to each other, so a blended UI would have half-worked and the split was deliberate. The plan was for v1 to go when v2 reached parity. The legacy removal deleted both, along with the Gradle project itself.
Decision: restore the v2 shell (`com.example.openair.v2`, plus `ui/theme`) and the Gradle project, and drop the v1 sources — `MainActivity`, `OpenAirViewModel`, `core/` and the v1 screens. `V2Activity` becomes the only launcher entry and takes `@string/app_name`.
Rationale: v1 was the half being kept for continuity with a desktop stack that no longer exists. With `openair-gui` gone, there is nothing on the other end of `_openair._tcp` for a phone to reach, so the v1 screen could only ever fail — and a launcher entry that cannot work is worse than no launcher entry. Keeping the Gradle project is not negotiable in the same way: it is the only thing that turns the gomobile `.aar` into an APK, and rewriting it later from memory would lose the signing config, the foreground-service declarations and the Compose setup that already work.
Alternatives considered: leaving the whole Android app deleted (rejected — it takes the phone, which is the device the protocol exists for, out of the project entirely). Restoring both stacks unchanged (rejected — see above; the second entry is dead weight that has to be explained to every user who taps it). Rewriting the shell against the current binding rather than restoring it (rejected — the restored shell tracks M1–M5 and builds against today's `mobile/` package unchanged, so a rewrite would spend a milestone's work reaching the same place).
Consequences: one launcher entry, named OpenAir; `app_name_v2` is gone from `strings.xml`. The v1 permissions stay because v2 needs the same ones. The APK is 39 MB debug-signed with two ABIs (arm64 + amd64), most of it `libgojni.so`; `OPENAIR_AAR_TARGETS=android/arm64` alone roughly halves it. A release APK still needs `keystore.properties`, which is not in version control, so an unsigned release build will not install — the debug APK is what a maintainer can put on a phone today.

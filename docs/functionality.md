...
Purpose: OpenAir 2.0. Per D-26 this is one module; `openair-gui` and `oabench` stay separate until v1.0 retires. **M1 (direct transfer) is implemented and works end to end** — a file moves between two devices over QUIC and its SHA-256 matches. No pairing (M2), no discovery (M3), no daemon (M4).

- `cmd/openair` — the CLI. `recv` listens and writes inbound files; `send` dials an explicit `host:port` and offers files. Both print the peer's device fingerprint and require confirmation, which is the whole of M1's security model since neither end pins a key in advance. `--yes` skips the prompts for automation. `main.go` dispatches, `common.go` holds key loading and the fingerprint/confirm helpers, `recv.go` and `send.go` are the two flows. Long term this becomes a client for the daemon over local IPC (D-29); it drives sessions directly only because M4 does not exist yet.
...
- A nil `session.Config.Authorize` admits every peer. Correct only for M1 (D-37); M2 must make it non-optional on the listening path.

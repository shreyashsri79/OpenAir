...
| ADR-5 Android core | D-10 → D-31 | accepted | gomobile-bound Go core, verified: binds quic-go in 24 s, 8.4 MB per ABI, idiomatic Java API. On-device throughput and battery still unmeasured. |
| ADR-6 Consensus/replication | D-11 | accepted | Dropped. Pairwise sessions need no agreement protocol. |
| ADR-8 Windows offload | D-22, D-23 | accepted | **New** — one Windows fast-path connection in the vendored quic-go, implementing USO (`UDP_SEND_MSG_SIZE`) and URO (`UDP_RECV_MAX_COALESCED_SIZE`), offered upstream. Windows currently falls back to `basicConn`: no send offload, no batched receive. |
